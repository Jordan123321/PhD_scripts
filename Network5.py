...
def Histplot(Histlist,namelist,name,units=None,strext=None):
...
def Histplot2(Histlist,namelist,name,units=None,strext=None):
    plt.figure(figsize=(20 ,20))
    gs = GridSpec(4,2,width_ratios=[1,1],height_ratios=[1,1,1,1],wspace=0.3,hspace=0.3)
    axlist=[]
    axlist.append( plt.subplot(gs[0,:]))
    ...
    for num,(D,G,I,S,O) in enumerate(zip(DAPIlst,GCGlst, INSlst, SSTlst,OALLlst)):
        ...
        for islt_n,islt in enumerate(skmeas.regionprops(IsltShpB1)):
            ...
            for gn, (G,Gtxt) in enumerate(zip([Gall,Galpha,Gbeta,Gdelta],['Overall','Alpha','Beta','Delta'])):
                ...
                f, ax = plt.subplots()
                ax.set_title('Bridges of %s network. Layout is truepos \n Islet is: %s, Image is: %s and patient is %s' %(Gtxt,islt_ns,numbr,pnt))
                ax.axis('off')
                nx.draw_networkx_edges(G,ax=ax,pos=posN,edgelist=brid,edge_color='k',width=5)
                nx.draw_networkx(G,ax=ax,pos=posN,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Bridge_Overlay'+ext, interpolation='none')
                plt.tight_layout()
                plt.close()
                ...
                f, ax = plt.subplots()
                ax.set_title('%s newtork of islet. Islet is: \n %s and patient is %s \n' %(Gtxt,numbr,pnt))
                ax.set_xlabel(r'X direction (pixels) $\rightarrow$'+'\n\n\n')
                ax.set_ylabel(r'$\leftarrow$ Y direction (pixels)', rotation='vertical')
                ax.imshow(Canvas.T, interpolation='none',cmap='gray')
                nx.draw_networkx(G,ax=ax,pos=posN,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                for txt in labtxt_orig:
                    ax.annotate(str(txt[2]), (txt[0], txt[1]),fontsize=5, bbox=dict(facecolor='white',lw=0,boxstyle="round", alpha=0.5))
                ax.set_xlim([0,NucPos4.shape[0]])
                ax.set_ylim([0,NucPos4.shape[1]])
                #plt.gca().invert_yaxis()
                f.text(0.5,.05, 'First Betti Number is :%i \n Second Betti Number is: %i' %(B1,B2),ha='right')
                f.tight_layout()
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Degree_Spat_Netw'+ext, interpolation='none')
                plt.close()
                ...
                f, ax = plt.subplots()
                ax.set_title('Degree %s network. Layout is truepos \n Islet is: %s, Image is: %s and patient is %s' %(Gtxt,islt_ns,numbr,pnt))
                ax.axis('off')
                nx.draw_networkx(G,ax=ax,pos=posN,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                for txt in labtxt_orig:
                    ax.annotate(str(txt[2]), (txt[0], txt[1]),fontsize=5, bbox=dict(facecolor='white',lw=0,boxstyle="round", alpha=0.5))
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Degree_Original_Network'+ext, interpolation='none')
                plt.tight_layout()
                plt.close()
                
                
                f, ax = plt.subplots()
                ax.set_title('Mean degree centrality of %s network. Layout is spring \n Islet is: %s, Image is: %s and patient is %s' %(Gtxt, islt_ns,numbr,pnt))
                ax.axis('off')
                nx.draw_networkx(G,ax=ax,pos=sp_pos,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                for txt in labtxt_spring:
                    ax.annotate(str(txt[2]), (txt[0], txt[1]),fontsize=5, bbox=dict(facecolor='white',lw=0,boxstyle="round", alpha=0.5))
    
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Degree_Spring_Network'+ext, interpolation='none')
                plt.tight_layout()
                plt.close()
                
                f, ax = plt.subplots()
                ax.set_title('Mean degree centrality of %s network. Layout is circular \n Islet is: %s, Image is: %s and patient is %s' %(Gtxt, islt_ns,numbr,pnt))
                ax.axis('off')
                nx.draw_networkx(G,ax=ax,pos=ci_pos,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                for txt in labtxt_circ:
                    ax.annotate(str(txt[2]),(txt[0], txt[1]),fontsize=5, bbox=dict(facecolor='white',lw=0,boxstyle="round", alpha=0.5))
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Degree_Mean_Circular_Network'+ext, interpolation='none')
                plt.tight_layout()
                plt.close()
                ...
                f, ax = plt.subplots()
                ax.set_title('Between centrality %s network. Layout is truepos \n Islet is: %s, Image is: %s and patient is %s' %(Gtxt,islt_ns,numbr,pnt))
                ax.axis('off')
                nx.draw_networkx(G,ax=ax,pos=posN,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                for txt in labtxt_orig:
                    ax.annotate(str(txt[2]), (txt[0], txt[1]),fontsize=5, bbox=dict(facecolor='white',lw=0,boxstyle="round", alpha=0.5))
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Between_Cent_Original_Network'+ext, interpolation='none')
                plt.tight_layout()
                plt.close()
                
                
                f, ax = plt.subplots()
                ax.set_title('Between centrality of %s network. Layout is spring \n Islet is: %s, Image is: %s and patient is %s' %(Gtxt, islt_ns,numbr,pnt))
                ax.axis('off')
                nx.draw_networkx(G,ax=ax,pos=sp_pos,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                for txt in labtxt_spring:
                    ax.annotate(str(txt[2]), (txt[0], txt[1]),fontsize=5, bbox=dict(facecolor='white',lw=0,boxstyle="round", alpha=0.5))
    
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Between_Cent_Spring_Network'+ext, interpolation='none')
                plt.tight_layout()
                plt.close()
                
                f, ax = plt.subplots()
                ax.set_title('Between centrality of %s network. Layout is circular \n Islet is: %s, Image is: %s and patient is %s' %(Gtxt, islt_ns,numbr,pnt))
                ax.axis('off')
                nx.draw_networkx(G,ax=ax,pos=ci_pos,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
                for txt in labtxt_circ:
                    ax.annotate(str(txt[2]),(txt[0], txt[1]),fontsize=5, bbox=dict(facecolor='white',lw=0,boxstyle="round", alpha=0.5))
                plt.savefig(opathsGph+'/'+numbr+'_'+islt_ns+'_'+Gtxt+'_Beteween_Cent_Circular_Network'+ext, interpolation='none')
                plt.tight_layout()
                plt.close()
