...
def Histplot2(Histlist,namelist,name,units=None,strext=None,bns=50,subbns=range(11)):
    plt.figure(figsize=(20 ,20))
    gs = GridSpec(4,2,width_ratios=[1,1],height_ratios=[1,1,1,1],wspace=0.3,hspace=0.3)
    axlist=[]
    axlist.append( plt.subplot(gs[0,:]))
...
ConcompArr=[[] for x in range(ntypes)]
ConcompYOCTLArr=[[] for x in range(ntypes)]
ConcompT1DArr=[[] for x in range(ntypes)]
ConcompT1DCTLArr=[[] for x in range(ntypes)]
ConcompT2DArr=[[] for x in range(ntypes)]
ConcompT2DCTLArr=[[] for x in range(ntypes)]
ConcompYOArr=[[] for x in range(ntypes)]
ConcompYOCTLArr=[[] for x in range(ntypes)]
    ...
    for num,(D,G,I,S,O) in enumerate(zip(DAPIlst,GCGlst, INSlst, SSTlst,OALLlst)):
        ...
        for islt_n,islt in enumerate(skmeas.regionprops(IsltShpB1)):
            ...
            for gn, (G,Gtxt) in enumerate(zip([Gall,Galpha,Gbeta,Gdelta],['Overall','Alpha','Beta','Delta'])):
                if nx.is_empty(G):
                    ConcompArr[gn].append(0)
                    if 'YOUNG' in f0.upper():
                        if 'CONTROL' in f0.upper():
                            ConcompYOCTLArr[gn].append(0)
                        else:
                            ConcompYOArr[gn].append(0)
                    elif 'T2D' in f0.upper():
                        if 'CONTROL' in f0.upper():
                            ConcompT2DCTLArr[gn].append(0)
                        else:
                            ConcompT2DArr[gn].append(0)
                    else:
                        if 'CONTROL' in f0.upper():
                            ConcompT1DCTLArr[gn].append(0)
                        else:
                            ConcompT1DArr[gn].append(0)
                    continue
                ...
                if 'YOUNG' in f0.upper():
                    if 'CONTROL' in f0.upper():
                        ...
                        ConcompYOCTLArr[gn].append(con_comp)
                    else:
                        ...
                        ConcompYOArr[gn].append(con_comp)
                elif 'T2D' in f0.upper():
                    if 'CONTROL' in f0.upper():
                        ...
                        ConcompT2DCTLArr[gn].append(con_comp)
                    else:
                        ...
                        ConcompT2DArr[gn].append(con_comp)
                else:
                    if 'CONTROL' in f0.upper():
                        ...
                        ConcompT1DCTLArr[gn].append(con_comp)
                    else:
                        ...
                        ConcompT1DArr[gn].append(con_comp)
...
for gn, Gtxt in enumerate(['Overall','Alpha','Beta','Delta']):
    ...
    Histplot2([ConcompArr[gn],
                      ConcompYOArr[gn], ConcompYOCTLArr[gn],
                      ConcompT1DArr[gn], ConcompT1DCTLArr[gn],
                      ConcompT2DArr[gn], ConcompT2DCTLArr[gn]],
                      stringlist,'Connected components',strext=Gtxt,
                      bns=range(0,max(ConcompArr[gn])+1),
                      subbns=range(0,max(ConcompArr[gn])+1))

    Histplot([ConcompArr[gn],
                      ConcompYOArr[gn], ConcompYOCTLArr[gn],
                      ConcompT1DArr[gn], ConcompT1DCTLArr[gn],
                      ConcompT2DArr[gn], ConcompT2DCTLArr[gn]],
                      stringlist,'Connected components',strext=Gtxt,
