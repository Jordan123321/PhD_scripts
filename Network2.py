...
#            OALLlst+=['/IDI images/'+i+'/'+f for f in os.listdir(ppaths+'/IDI images/'+i) if (f.endswith('.tif') and 'OVERLAY' in f.upper())]
    ...
    for num,(D,G,I,S,O) in enumerate(zip(DAPIlst,GCGlst, INSlst, SSTlst,OALLlst)):
        ...
        if Silent == False:
            ...
            f.text(0.5,.05, 'First Betti Number is :%i \n Second Betti Number is: %i' %(B1,B2),ha='right')
            f.tight_layout()
            plt.savefig(opathsGph+'/'+numbr+'_Spat_Netw'+ext, interpolation='none')
            plt.close()
        ...
        sp_pos=nx.spring_layout(G,pos=posdict, k=1000/math.sqrt(len(NPosdf['c'].tolist())))
        ...
        plt.savefig(opathsGph+'/'+numbr+'_Spring_Network'+ext, interpolation='none')
        plt.tight_layout()
        plt.close()
        ...
        plt.savefig(opathsGph+'/'+numbr+'_Circular_Network'+ext, interpolation='none')
        plt.tight_layout()
        plt.close()
        
        f, ax = plt.subplots()
        ax.set_title('Network representation of islet. Layout is planar \n Islet is: %s and patient is %s' %(numbr,pnt))
        ax.axis('off')
        nx.draw_networkx(G,ax=ax,pos=pl_pos,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
        plt.savefig(opathsGph+'/'+numbr+'_Planar_Network'+ext, interpolation='none')
        plt.tight_layout()
        plt.close()
        
        f, ax = plt.subplots()
        ax.set_title('Network representation of islet. Layout is Kamada Kawai \n Islet is: %s and patient is %s' %(numbr,pnt))
        ax.axis('off')
        nx.draw_networkx(G,ax=ax,pos=kk_pos,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
        plt.savefig(opathsGph+'/'+numbr+'_Kamada_Kawai_Network'+ext, interpolation='none')
        plt.tight_layout()
        plt.close()
        
        f, ax = plt.subplots()
        ax.set_title('Network representation of islet. Layout is spectral \n Islet is: %s and patient is %s' %(numbr,pnt))
        ax.axis('off')
        nx.draw_networkx(G,ax=ax,pos=spec_pos,with_labels=False,node_color=ncol,edge_color=ecol,node_size=50,width=1.5)
        plt.savefig(opathsGph+'/'+numbr+'_Spectral_Network'+ext, interpolation='none')
        plt.tight_layout()
        plt.close()
