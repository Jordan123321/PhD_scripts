...
def DiameterCalc(f_image,path):
    f_image2=skmorph.erosion(f_image,selem=skmorph.disk(1))
    f_image2=f_image^f_image2
    f_image3=skmorph.erosion(f_image,selem=skmorph.disk(3))
    f_image4=f_image^f_image3
    X_coord,Y_coord=f_image2.nonzero()
    MDist=0
    x0,y0,x1,y1=0,0,0,0
    while X_coord != []:
        i,j=X_coord[0],Y_coord[0]
        X_coord,Y_coord=X_coord[1:],Y_coord[1:]
        X_coord_tmp,Y_coord_tmp=X_coord-i,Y_coord-j

        Euclid_dist=np.add(np.multiply(X_coord_tmp,X_coord_tmp),np.multiply(Y_coord_tmp,Y_coord_tmp))
        MDist0,Mind=Euclid_dist.max(),Euclid_dist.argmax()
        if MDist0>MDist:
            MDist=MDist0
            x0,y0,x1,y1=i,j,X_coord[Mind],Y_coord[Mind]
    #for i,j in zip(X_coord,Y_coord):
    #    MDist0=((X_coord-i)*(X_coord-i)+(Y_coord-j)*(Y_coord-j)).max()
    #    MDist=max(MDist,MDist0)

    if Silent == False:
        f, ax = plt.subplots()
        ax.set_title(r'Halo with max dist potted. Islet is: %s and patient is %s' %(numbr,pnt))
        ax.set_xlabel(r'X direction (pixels) $\rightarrow$ ')
        ax.set_ylabel(r'$\leftarrow$ Y direction (pixels)', rotation='vertical')
        ax.imshow(f_image2, interpolation='none')
        ax.plot([y0,y1],[x0,x1],'o-')
        plt.savefig(path, interpolation='none')
        plt.close()

    return f_image4,math.sqrt(MDist)


def round_sig(x, sig=3):
    return math.round(x, sig-int(math.floor(math.log10(abs(x))))-1)
...
def Histplot2(Histlist,namelist,name,units=None):
    plt.figure(figsize=(20 ,20))
    gs = GridSpec(4,2,width_ratios=[1,1],height_ratios=[1,1,1,1],wspace=0.3,hspace=0.3)
    axlist=[]
    axlist.append( plt.subplot(gs[0,:]))
    ...
    for n, (i,j,k) in enumerate(zip(Histlist,axlist,namelist)):
        ...
        j.set_xticklabels(["{:.2e}".format(t) for t in j.get_xticks()])
    #plt.tight_layout()
    plt.savefig(path2+'/'+name+'_equalised_x_axis'+ext, interpolation='none')
    plt.close()
    ...
    for num,(D,G,I,S,O) in enumerate(zip(DAPIlst,GCGlst, INSlst, SSTlst,OALLlst)):
        ...
        for p in props:
            ...
            if(
                ...
                opathsOlay2=opaths+'/Olay2'
                if not os.path.exists(opathsOlay2):
                    os.makedirs(opathsOlay2)
                strOlay2=opathsOlay2+'/'+numbr+'_'+str(islcount)+'Diam_Count'+ext
                Avrad=(0.5*p.minor_axis_length+0.5*p.major_axis_length)*Rat
                ExRad=ExCirc(p.filled_image,(p.centroid[0]-p.bbox[0],p.centroid[1]-p.bbox[1]))
                InRad=InCirc(p.filled_image,(p.centroid[0]-p.bbox[0],p.centroid[1]-p.bbox[1]))
                Imolay, Diameter=DiameterCalc(p.filled_image,strOlay2)
                X_coord,Y_coord=Imolay.nonzero()
                X_coord+=p.bbox[0]
                Y_coord+=p.bbox[1]
                Diameter=Diameter*Rat
                CircRat=InRad/ExRad
                Area=p.area*Rat*Rat
                Perim=p.perimeter*Rat
                Isoparametricratio=4*math.pi*Area/(Perim*Perim)
                PerimRat=(2*math.pi*Avrad)/Perim
                ...
                Oim1[X_coord,Y_coord,:]=(255,0,255)
                #Oim1[p.bbox[0]:p.bbox[2],p.bbox[1]:p.bbox[3],0]=Imolay*255
                #Oim1[p.bbox[0]:p.bbox[2],p.bbox[1]:p.bbox[3],1]=Imolay*255
                #Oim1[p.bbox[0]:p.bbox[2],p.bbox[1]:p.bbox[3],2]=Imolay*255
            ...
            if Silent == False:
                ...
                ax.set_xlabel(r'X direction (pixels) $\rightarrow$ ')
                ax.set_ylabel(r'$\leftarrow$ Y direction (pixels)', rotation='vertical')
                ax.imshow(Oim1, interpolation='none')
                ax.plot(p.centroid, 'o', c='w')
...
Histplot2([IsoparametricratioArr,
