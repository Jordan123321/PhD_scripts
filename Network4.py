...
path2=path0+ '/Output_Network_Metric'
ext='.svg'
...
#            OALLlst+=['/IDI images/'+i+'/'+f for f in os.listdir(ppaths+'/IDI images/'+i) if (f.endswith('.tif') and 'OVERLAY' in f.upper())]
    ...
    for num,(D,G,I,S,O) in enumerate(zip(DAPIlst,GCGlst, INSlst, SSTlst,OALLlst)):
        ...
        for islt_n,islt in enumerate(skmeas.regionprops(IsltShpB1)):
            ...
            cent_mean=statistics.mean([*centrality.values()])
            
            
            
            labtxt_orig=[[x[1][1],x[1][0],round(y[1],2)] for x,y in zip(pos.items(),centrality.items()) if x[0]==y[0]]
            labtxt_spring=[[x[1][0],x[1][1],round(y[1],2)] for x,y in zip(sp_pos.items(),centrality.items()) if x[0]==y[0]]
            labtxt_circ=[[x[1][0],x[1][1],round(y[1],2)] for x,y in zip(ci_pos.items(),centrality.items()) if x[0]==y[0]]
            ...
            f, ax = plt.subplots()
            ax.set_title('Eigenvector centrality of islet. Layout is truepos \n Islet is: %s, Image is: %s and patient is %s' %(islt_ns,numbr,pnt))
            ax.axis('off')
            ...
            f, ax = plt.subplots()
            ax.set_title('Eigenvector centrality of islet. Layout is spring \n Islet is: %s, Image is: %s and patient is %s' %(islt_ns,numbr,pnt))
            ax.axis('off')
            ...
            f, ax = plt.subplots()
            ax.set_title('Eigenvector centrality of islet. Layout is circular \n Islet is: %s, Image is: %s and patient is %s' %(islt_ns,numbr,pnt))
            ax.axis('off')
            ...
            CentMeanArr.append(cent_mean)
            if 'YOUNG' in f0.upper():
                if 'CONTROL' in f0.upper():
                    CentMeanYOCTLArr.append(cent_mean)
                else:
                    CentMeanYOArr.append(cent_mean)
            elif 'T2D' in f0.upper():
                if 'CONTROL' in f0.upper():
                    CentMeanT2DCTLArr.append(cent_mean)
                else:
                    CentMeanT2DArr.append(cent_mean)
            else:
                if 'CONTROL' in f0.upper():
                    CentMeanT1DCTLArr.append(cent_mean)
                else:
                    CentMeanT1DArr.append(cent_mean)
...
Histplot2([CentMeanArr,
                  CentMeanYOArr, CentMeanYOCTLArr,
                  CentMeanT1DArr, CentMeanT1DCTLArr,
                  CentMeanT2DArr, CentMeanT2DCTLArr],
                  stringlist,'Mean centrality')

Histplot([CentMeanArr,
                  CentMeanYOArr, CentMeanYOCTLArr,
                  CentMeanT1DArr, CentMeanT1DCTLArr,
                  CentMeanT2DArr, CentMeanT2DCTLArr],
                  stringlist,'Mean centrality')
