...
#            OALLlst+=['/IDI images/'+i+'/'+f for f in os.listdir(ppaths+'/IDI images/'+i) if (f.endswith('.tif') and 'OVERLAY' in f.upper())]
    ...
    for num,(D,G,I,S,O) in enumerate(zip(DAPIlst,GCGlst, INSlst, SSTlst,OALLlst)):
        ...
        nmbr=str(num+1)
        Dim0=skio.imread(ppaths+'/'+D)
        Gim0=skio.imread(ppaths+'/'+G)
        Iim0=skio.imread(ppaths+'/'+I)
        Sim0=skio.imread(ppaths+'/'+S)
        Oim0=skio.imread(ppaths+'/'+O)
        xRat,yRat=Oim0.shape[0]/xRes,Oim0.shape[1]/yRes
        avgRat=(xRat+yRat)/2
        Dim=sktrans.resize(Dim0,(xRes,yRes))
        Gim=sktrans.resize(Gim0,(xRes,yRes))
        Iim=sktrans.resize(Iim0,(xRes,yRes))
        Sim=sktrans.resize(Sim0,(xRes,yRes))
        Oim=sktrans.resize(Oim0,(xRes,yRes))
        ...
        ##Smooth it
        DiffSim2=filt.gaussian(DiffSim1,1/avgRat)
        #Filter it
        ...
        IsltShp1=filt.gaussian(IsltShp0,10/avgRat)
        IsltShp2=IsltShp1>filt.threshold_triangle(IsltShp1)
        ##Get rid of any small objects that may yet exist
        IsltShp3=skmorph.remove_small_holes(IsltShp2, connectivity=1, min_size=1000/avgRat)
        IsltShp4=skmorph.remove_small_objects(IsltShp3, connectivity=1, min_size=10000/avgRat)
        ...
        ##Local threshold
        NucPos=(Nuc>filt.threshold_local(Nuc, block_size=101/avgRat))
        ...
        NucPos3=skmorph.remove_small_holes(IsltNuc, connectivity=1, min_size=1000/avgRat)
        NucPos4=skmorph.remove_small_objects(NucPos3, connectivity=1, min_size=100/avgRat)
        ##Erode the image, to disentangle joint up DAPI stains
        NucPos5=NucPos4
        #NucPos5=skmorph.erosion(NucPos4, selem=skmorph.disk(1))
        ...
        for p in props:
            ...
            ##Green is delta cells
                ...
                SmS1+=1
                CellType.append('d')
            for crds in p.coords:
                Celltype.append(crd[0]*xRes+crd[1])
        ArrG1.append(SmG1)
        ArrI1.append(SmI1)
        ArrS1.append(SmS1)
        Canvas1=Canvas1*255
