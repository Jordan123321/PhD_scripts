...
path2=path0+ '/Output_Barcode/Chap2/Individ_T1D_CTL'
...
res=100
SSpace=np.linspace(start=1,stop=199,num=res)
cval=101
...
for ssn,ss in enumerate(SSpace):
    ##Local threshold
    NucPos=(Nuc>filt.threshold_local(Nuc, block_size=ss))
...
ax.set_xlabel(r'Local Threshold Pixel Block Size Parameter Value', fontsize=25)
...
plt.tight_layout()
plt.savefig(path2+'/Individ_Local_Filt'+ext, interpolation='none')
plt.close()