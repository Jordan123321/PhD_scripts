...
for ssn,ss in enumerate(SSpace):
    ...
    B0[ssn]=skmeas.label(IsltShp4, return_num=1)[1]
    B1[ssn]=skmeas.label(IsltShp4, return_num=1)[1]
...
ax.set_ylim(0,1.1*B0.max())
plt.tight_layout()
plt.savefig(path2+'/Individ_Gauss_Blur_B0'+ext, interpolation='none')
plt.close()
...
ax.axvline(cval,ls='--',c='r')
ax.set_xlim(SSpace[0],SSpace[-1])
ax.set_ylim(0,1.1*B1.max())
plt.tight_layout()
plt.savefig(path2+'/Individ_Gauss_Blur_B1'+ext, interpolation='none')
plt.close()